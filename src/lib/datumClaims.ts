...
import { Contract, Signer, Provider, AbiCoder, keccak256 } from "ethers";
...
function computeClaimHash(args: {
  ...
  return keccak256(AbiCoder.defaultAbiCoder().encode(
    ...
    [args.campaignId, args.publisher, args.user, args.eventCount, args.rateWei,
      args.actionType, ZERO_HASH /*clickSessionHash*/, args.nonce, args.prevHash, ZERO_HASH /*stakeRootUsed*/],
  ));
}
...
export async function submitClaim(
  ...
  const [last, prevHash, rateWei, expectedRelaySigner, head, powEnforced] = await Promise.all([
  ...
  // Per-impression PoW (enforced live for every action type). Mine a nonce bound
  // to the exact claim hash the contract will derive (nonce = firstNonce,
  // prevHash from chain). Without it the claim is rejected (reason 27).
  ...
  if (powEnforced) {
    ...
    const claimHash = computeClaimHash({
      ...
      rateWei, actionType, nonce: firstNonce, prevHash,
    });
    const mined = minePowNonce(claimHash, target);
    if (!mined) return { ok: false, status: 0, message: "PoW too hard — try fewer impressions" };
    powNonce = mined;
  }
    ...
    claims: [
      {
        ...
        actionType,
        // proof sidecar — carries the mined PoW nonce (empty if PoW disabled).
        proof: powEnforced
          ? [{
              clickSessionHash: ZERO_HASH,
              stakeRootUsed: ZERO_HASH,
              nullifier: ZERO_HASH,
              powNonce,
              zkProof: Array(8).fill(ZERO_HASH),
              actionSig: Array(3).fill(ZERO_HASH),
            }]
          : [],
      },
    ],
